...
//Rotate代码
	...
	//var u_MvpMatrix = gl1.getUniformLocation(gl.program, 'u_MvpMatrix');
...
//HUD代码
	var tick = function() {   // Start drawing
		draw2D(ctx); // Draw 2D
		draw(gl, viewProjMatrix, null, currentAngle);
...
function render() {
	...
	gl.useProgram(programObj);
	mvStack.push(matMV); 
	matMV = mult(matMV, translate(3.0, -0.5, -3.0));
	matMV = mult(matMV, rotateY(-yRot * 3.0));
...
function draw2D(ctx, currentAngle) {
	ctx.clearRect(0, 0, 300, 300); // Clear <hud>
	// Draw triangle with white lines
	ctx.beginPath();                      // Start drawing
	ctx.moveTo(300, 10); ctx.lineTo(300, 150); ctx.lineTo(40, 150); ctx.lineTo(40, 10);
	ctx.closePath();
	...
	ctx.fillText('HUD', 45, 30); 
	ctx.fillText('No._NAME', 45, 50);
	
	ctx.fillText('Press W A S D to control the Move', 45, 95); 
	ctx.fillText('Press Space to control the Jump', 45, 115);
	ctx.fillText('Press 1 2 3 to control the Light', 45, 135);
}
//Rotate代码
